...
#define NAME "FusionTweaker"
#define DISPLAY_NAME "FusionTweaker service"
...
class FusionTweakerService;
typedef FusionTweakerService ServiceType;

class FusionTweakerService : public BaseService
{
public:

	FusionTweakerService(SERVICE_STATUS_HANDLE statusHandle, const SERVICE_STATUS& status) :
...
private:
	...
	{
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\FusionTweaker", 0, KEY_QUERY_VALUE, &_key) != ERROR_SUCCESS)
			return false;
